  ...
  if(req.method === "POST") {
    ...
    const emailValid = regex.test(email)
      ...
      if(!firstName ||
        ...
        try{
          client = await MongoClient.connect(`mongodb+srv://hilary:august11@node-app.hnwnorp.mongodb.net/address?retryWrites=true&w=majority`)
        } catch(err) {
          res.status(500).json({
        ...
        try{
          const result = await db.collection("address").insertOne(newAddress);
          newAddress.id = result.insertId;
        } catch(err) {
          client.close()
          res.status(500).json({
