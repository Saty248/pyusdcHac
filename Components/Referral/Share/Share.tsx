...
const Share: React.FC<ShareProps> = ({ activeSection, section, isMobile, referralCode, user }) => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => setIsCopied((prev) => ({ ...prev, code: false })), 2000);
    return () => clearTimeout(timeoutId);
  }, [isCopied.code]);

  useEffect(() => {
    if (!isCopied.link) return;
    const timeoutId = setTimeout(() => setIsCopied((prev) => ({ ...prev, link: false })), 2000);
    return () => clearTimeout(timeoutId);
  }, [isCopied.link]);
    ...
    <div className="flex flex-wrap gap-8">
      <div className="flex flex-col gap-[15px] px-[51px]">
        ...
        <p className="text-[#87878D] text-[15px] font-normal">
          You can also share your referral link or code by copying and sending it or sharing it on your social media. You can define your code once by entering your preferred value and pressing enter.
        </p>
        <div className="flex flex-wrap gap-[21px]">
          ...
          <div className="flex gap-[9px] flex-wrap justify-between">
            <CopyableInput
              ...
              handleCopy={(e) => handleCopy(e, `${origin}/r/${referralCode}`, false)}
              disabled={true}
            />
            <SocialShareButton
              platform="facebook"
              onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${origin}/r/${referralCode}`, "_blank")}
            >
              <FacebookIcon />
            </SocialShareButton>
            <SocialShareButton
              platform="linkedin"
              onClick={() => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${origin}/r/${referralCode}`, "_blank")}
            >
              <LinkedInIcon />
            </SocialShareButton>
            <SocialShareButton
              platform="twitter"
              onClick={() => window.open(`https://twitter.com/intent/tweet?text=${origin}/r/${referralCode}`, "_blank")}
            >
              <XIcon />
            </SocialShareButton>
          </div>
