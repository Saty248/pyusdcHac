...
const CopyableInput: React.FC<CopyableInputProps> = ({
  ...
  <div className="relative w-full md:w-[300px]">
    ...
    />
    {canCopy && (
      <p onClick={handleCopy} className="absolute right-[22px] top-1/2 -translate-y-1/2 text-[#0653EA] text-[14px] cursor-pointer">
        {isCopied ? "Copied ✓" : isReferralLink ? 'Copy code ' : 'Copy link'}
      </p>
    )}
    {canChangeCode && (
      <p onClick={handleUpdateReferralCode} className="absolute right-[22px] top-1/2 -translate-y-1/2 text-[#0653EA] text-[14px] cursor-pointer">
        {"Update code"}
      </p>
    )}
  </div>
