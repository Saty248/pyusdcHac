...
const MobileNavbar = (showMobileNavbar) => {
    ...
    const isActive = href ? asPath.includes(href) : false;
        ...
        if (onClick !== undefined) {
          return (
            <div title={text} onClick={onClick} className={`${style || ''} cursor-pointer py-[7.32px] flex items-center gap-[14.64px] px-[14.64px] w-full hover:text-[#4285F4] hover:bg-[#E9F5FE] hover:font-semibold ${isActive && 'bg-[#E9F5FE] text-[#4285F4]'} rounded-[3.66px] hidden`}>
  ...
  return (
    ...
    <div className="fixed top-[6.5rem] bg-white w-screen z-50 flex flex-col rounded-t-3xl overflow-y-auto  h-screen" style={{ width: '100%', height: '90%' }}>
      <div>  
       <div className=' h-[13%] flex flex-col justify-center items-center gap-7'>
           <p onClick={handleMenuClick} className='border-4 border-dark-grey w-[20%] rounded-md'></p>
            <p className='font-medium text-xl'>Menu</p>
       </div>
       <div>
        <div className=' px-6 flex flex-col gap-[23px] text-lg'>
